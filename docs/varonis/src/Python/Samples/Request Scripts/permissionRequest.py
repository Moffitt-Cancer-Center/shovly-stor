...
def parse_arguments():
    """Parse command-line arguments for permission request fields"""
    parser = argparse.ArgumentParser(description='Submit a permission request to DAG')
    ...
    # Optional permission request fields (either resource-path OR resource-id required)
    parser.add_argument('--resource-path', help='Display path of the resource/folder')
    parser.add_argument('--resource-id', type=int, help='ID of resource/folder (integer), should only be called by other scripts')
    parser.add_argument('--permission-type', help='Permission type (e.g., Read, Write, Full Control)')
...
def handleInputs(args=None):
    ...
    if args:
        ...
        resourceDisplayPath = args.resource_path
        resourceId = args.resource_id
        permissionTypeName = args.permission_type.title() if args.permission_type else None
        ...
        # Either resource path OR resource ID should be provided (not both)
        has_resource = bool(resourceDisplayPath) or bool(resourceId)
        if (has_resource and permissionTypeName and requesteeDomainName and 
            requesteeAccountName and accessAction and reason):
            command_line_mode = True
    else:
        # Initialize all as None for interactive mode
        resourceDisplayPath = permissionTypeName = resourceId = None
        requesteeAccountName = requesteeDomainName = None
        accessAction = reason = None
        isAutoApprove = expirationDate = activationDate = expirationDaysInterval = None

    # Interactive prompts for missing required fields
    # Either resource path OR resource ID must be provided
    if not resourceDisplayPath and not resourceId:
        while not resourceDisplayPath:
            resourceDisplayPath = input(r'Display Path of Folder: ').strip()
            if not resourceDisplayPath:
                log.error("Display Path is required and cannot be empty.")
            
    while not permissionTypeName:
        permissionTypeName = input(r'Permission Type requested: ').strip()
        if permissionTypeName:
            permissionTypeName = permissionTypeName.title()
        else:
            log.error("Permission Type is required and cannot be empty.")
    ...
    reqVars = {
        "resourceDisplayPath": resourceDisplayPath,
...
def main(return_results=False, reqVars=None):
    ...
    if reqVars is None:
        ...
        # First validate resource specification if any resource arguments are provided
        if args.resource_path and args.resource_id:
            log.error("Cannot specify both --resource-path and --resource-id. Please use only one.")
            sys.exit(1)
        
        # Check if all required fields are provided via command line
        has_resource = bool(args.resource_path) or bool(args.resource_id)
        all_required_provided = (
            has_resource and args.permission_type and args.user and args.action and args.reason
        )
        ...
        elif any([args.resource_path, args.resource_id, args.permission_type, args.user, args.action, args.reason]):
            log.error("When providing request fields via command line, all required fields must be provided: either --resource-path OR --resource-id, plus --permission-type, --user, --action, and --reason")
            sys.exit(1)
    else:
    ...
    if reqVars is None:
        ...
        log.info("Using provided reqVars for permission request")
        log.info(f"Request variables: {json.dumps(reqVars, indent=2)}")

    #open the mutation query file and bring it into a var 
    query = helpers.getQueryFile('async_submitPermissionRequest',graphqlfolder)
    #run the query 
    log.debug("Executing GraphQL mutation: async_submitPermissionRequest")
    ...
    job_id = f"{result['createGovernedPermissionRequestsAsync']['jobId']}"
