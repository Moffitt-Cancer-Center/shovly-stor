...
"@
    ...
    # GraphQL mutation to remove members from group
    $mutation = @"
...
"@
    ...
    try {
        ...
        Write-Host "`nERROR: Failed to execute remove membership mutation. $_" -ForegroundColor Red
        Write-Host "`nMutation that was attempted:" -ForegroundColor Yellow
        Write-Host $mutation -ForegroundColor Cyan
        exit 1
    }
...
try {
    ...
    Write-Host "`nRemoving members from group..."
    $removeResult = Remove-GroupMembership -Config $config -InputMethod $inputMethod -GroupInfo $groupInfo -MembersInfo $membersInfo
