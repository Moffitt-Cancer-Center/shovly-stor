...
<#
.SYNOPSIS
Queries Varonis API for enabled but stale users.

.DESCRIPTION
This script searches for user accounts that are enabled but have been inactive (stale)
for an extended period. These accounts may represent security risks.
...
# Constants
$GRAPHQL_QUERY_FILENAME = "Identities - Enabled but Stale Users.gql"
$OUTPUT_FILENAME = "Identities-EnabledButStaleUsers.json"
$QUERY_DESCRIPTION = "Enabled but Stale Users"
...
Write-Host "Searching for enabled but stale user accounts"
