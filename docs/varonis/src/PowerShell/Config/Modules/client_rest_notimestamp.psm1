...
class RestClient {
    ...
    [Microsoft.PowerShell.Commands.WebResponseObject] Get([string]$endpoint,[string]$topath,[string]$filename) {
        ...
        try {
            $trimmedFilename = $this.TrimTimestamp($filename)
            return Invoke-WebRequest -Uri $url -Headers $this.Headers -UseBasicParsing -OutFile $topath\$trimmedFilename
    ...
    [string] TrimTimestamp ([string]$filename) {
        $prepFilename = [System.IO.Path]::GetFileNameWithoutExtension($filename)
        $prepFileExtension = [System.IO.Path]::GetExtension($filename)
        $trimmedFilename = $prepFilename -replace '-\d{8}-\d{6}$',''
        return "$trimmedFilename$prepFileExtension"
    }
