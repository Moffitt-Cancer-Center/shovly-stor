...
param(
    [Parameter(Mandatory=$true, HelpMessage="Set the query type from one of Resources or Groups")]
    [ValidateSet("Resources", "Groups")]
    [string]$querytype,
...
$logFileName = "Transcript-$envUser-$envHost-$currDate-PS$psVersion _DAG_getAutoRules.log"
...
if ($querytype -eq "Groups") {
    Write-Host "Groups not yet Supported" -ForegroundColor Red
}
elseif ($querytype -eq "Resources") {
    Write-Host "Get Resource Automatic Rules from API" -ForegroundColor Green
    $asyncquery_file = Join-Path $graphqlPath "queryjob_getAutoRules.graphql"
    $checkstatusquery_file = Join-Path $graphqlPath "CheckStatus_getAutoRules.graphql"
    $reportquery_file = Join-Path $graphqlPath "result_getAutoRules.graphql"
}
...
if ($querytype -eq "Resources") {
    $id = $jobID.governedResourceAutomaticRulesAsync.jobId
}
...
if($querytype -eq "Resources") {
    $checkstatus = $jobstatus.governedResourceAutomaticRulesQueryJob.jobStatus
}
...
if($querytype -eq "Resources") {
    $result = $resultlist.governedResourceAutomaticRulesQueryJob.results
}
