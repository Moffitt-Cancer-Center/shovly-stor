...
# Build queryVariables with the required data source and start date parameter values
$queryVariables = @{
    data_source_id = $dataSourceId
    gte = $startDate.ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ss.fffZ")
}

# Add the optional end date parameter if provided
if ($null -ne $endDate) {
    $queryVariables.lt = $endDate.ToUniversalTime().ToString("yyyy-MM-ddTHH:mm:ss.fffZ")
}

# Define the GraphQL query file path
$QueryFilePath = Join-Path $PSScriptRoot "..\Gql\Events\eventsExport.gql"
if (-not (Test-Path $QueryFilePath)) {
    throw "GraphQL file not found: $QueryFilePath"
}

# Call Common\Export.ps1 with ExportType=Events and QueryVariables
...
& $exportScript -EntityName "Events" -QueryFilePath $QueryFilePath -QueryVariables $queryVariables
