...
<#
.SYNOPSIS
Queries Varonis API for permissions on data exposed to the whole organization.

.DESCRIPTION
This script searches for permission entries on data that is exposed to the entire organization,
helping identify potential overexposure risks.
...
.\PhysicalPermissions-Get-PermissionsOnDataExposedToOrganization.ps1 -dataSourceIds "1,5,10"
#>
...
# Constants
$GRAPHQL_QUERY_FILENAME = "PhysicalPermissions - Permissions on Data Exposed to the Whole Organization.gql"
$OUTPUT_FILENAME = "PhysicalPermissions-PermissionsOnDataExposedToOrganization.json"
$QUERY_DESCRIPTION = "Permissions on Data Exposed to the Whole Organization"
...
try {
    ...
    $results = Invoke-PhysicalPermissionsQuery -Query $baseQuery -Config $config -DataSourceIds $dataSourceIdsArray -QueryDescription $QUERY_DESCRIPTION

    # Save the results using the shared helper function
    Save-Results -Results $results -DestinationFolder $destinationFolder -FileName $OUTPUT_FILENAME -ResultType "permission entries"
