...
<#
.SYNOPSIS
    Submit a permission request to DAG

.DESCRIPTION
    This script submits a permission request to grant or revoke access to a folder/resource in DAG.
...
.PARAMETER ResourcePath
    Display path of the resource/folder

.PARAMETER PermissionType
    Permission type (e.g., Read, Write, Full Control)
...
.EXAMPLE
    .\permissionRequest.ps1 -Origin "https://tenant.varonis.io" -ApiKey "vkey_..." -ResourcePath "\\server\share\folder" -PermissionType "Read" -User "DOMAIN\UserName" -Action Grant -Reason "Need access for project"

.EXAMPLE
    .\permissionRequest.ps1 -ResourcePath "\\server\share\folder" -PermissionType "Write" -User "DOMAIN\UserName" -Action Grant -Reason "Need write access" -AutoApprove -ExpirationDays 30
#>
...
param(
    ...
    [Parameter(Mandatory=$false, HelpMessage="Display path of the resource/folder")]
    [string]$ResourcePath,

    [Parameter(Mandatory=$false, HelpMessage="Permission type (e.g., Read, Write, Full Control)")]
...
$logFileName = "Transcript-$envUser-$envHost-$currDate-PS$psVersion _DAG_permissionRequest.log"
$logFilePath = Join-Path -Path $logFolder -ChildPath $logFileName

# Start the transcript
Start-Transcript -Path $logFilePath

Write-Host "=== DAG Permission Request Script ===" -ForegroundColor Cyan
Write-Host ""
...
if ($ResourcePath -and $PermissionType -and $User -and $Action -and $Reason) {
    ...
    if ($userDomain -and $userAccount) {
        ...
        Write-Host "Invalid user format in command line parameters" -ForegroundColor Red
        Stop-Transcript
        exit 1
    }
}
elseif ($ResourcePath -or $PermissionType -or $User -or $Action -or $Reason) {
    Write-Host "When providing request fields via command line, all required fields (-ResourcePath, -PermissionType, -User, -Action, -Reason) must be provided" -ForegroundColor Red
    Stop-Transcript
    exit 1
}
...
if (-not $commandLineMode) {
    while ([string]::IsNullOrWhiteSpace($ResourcePath)) {
        $ResourcePath = Read-Host "Display Path of Folder"
        if ([string]::IsNullOrWhiteSpace($ResourcePath)) {
            Write-Host "Display Path is required and cannot be empty." -ForegroundColor Red
        }
    }
}
...
if (-not $commandLineMode) {
    while ([string]::IsNullOrWhiteSpace($permissionTypeName)) {
        $permissionTypeName = Read-Host "Permission Type requested"
        if ([string]::IsNullOrWhiteSpace($permissionTypeName)) {
            Write-Host "Permission Type is required and cannot be empty." -ForegroundColor Red
        }
    }
}

# Normalize Permission Type to Title Case
if (-not [string]::IsNullOrWhiteSpace($permissionTypeName)) {
    # Convert to Title Case (e.g., "read" -> "Read", "full control" -> "Full Control")
    $textInfo = (Get-Culture).TextInfo
    $permissionTypeName = $textInfo.ToTitleCase($permissionTypeName.ToLower())
}
...
$variables = @{
    resourceDisplayPath = $ResourcePath
...
#region Execute Permission Request

Write-Host ""
Write-Host "Submitting permission request..." -ForegroundColor Yellow

# Load the mutation query
$asyncMutationFile = Join-Path $graphqlPath "async_submitPermissionRequest.graphql"
...
try {
    ...
    $jobId = $mutationResult.createGovernedPermissionRequestsAsync.jobId
    $jobStatus = $mutationResult.createGovernedPermissionRequestsAsync.jobStatus
    ...
    Write-Host "Error executing permission request mutation: $($_.Exception.Message)" -ForegroundColor Red
    Stop-Transcript
    exit 1
}
